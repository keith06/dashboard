...
function dashboardExists($db, $code)
{
    $q = $db->prepare("SELECT id FROM dashboard WHERE code = ?");
    $q->execute(array($code));
    $dashboard = $q->fetchAll();

    return sizeof($dashboard) == 0 ? false : $dashboard[0];
}


function checkNewEvents($db, $dashboard_id, $user_id, $last_update_id)
{
    $q = $db->prepare("SELECT * FROM event WHERE dashboard_id = ? AND user_id != ? AND id > ? ORDER BY date_received ASC");
    $q->execute(array($dashboard_id, $user_id, $last_update_id));
